 ...
 */
class ViewFilterPromotionSearchApi extends BooleanOperator {
  /**
   * The current user.
   *
  ...
  function query() {
    ...
    // $mysqlquery->innerJoin('commerce_product__variations', 'cpv',
    // "cpv.entity_id = commerce_product_field_data.product_id");
    ...
    // $productsPromotion = [];
    // if ($variationsIds) {
    // $purchasable_entities =
    // \Drupal::entityTypeManager()->getStorage('commerce_product_variation')->loadMultiple($variationsIds);
    // foreach ($purchasable_entities as $variation_id => $purchasable_entity) {
    // $result = $this->priceCalculator->calculate($purchasable_entity, 1,
    // $this->getContext(), $this->getAjustements());
    ...
    // $query->addWhere('AND', 'cpv.variations_target_id', $productsPromotion,
    // 'IN');
    // }
    // }
  }
